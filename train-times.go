...
type TrainTimesArrivals struct {
	Trains  []TrainTimesArrival `json:"trains"`
	Updated string              `json:"updated"`
}

func GetTrainTimesArrival(cityCode string) (*TrainTimesArrivals, error) {
	url := fmt.Sprintf("http://www.gares-sncf.com/fr/train-times/%s/arrival", cityCode)
	request := gorequest.New()
	var result TrainTimesArrivals
